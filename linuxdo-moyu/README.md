...
- Excel 侧边栏导航隐藏滚动条（仍可用滚轮滚动）

### v1.8.1

- 详情正文字号调回 14px；单元格内边距适度收紧，避免空白过大

### v1.8.0

- 详情楼层更扁平紧凑：作者列收窄、正文行距/内边距压缩、操作栏弱化；「紧凑模式」贯通详情
- 隐藏楼内图片时改为 `[图]` / `[图×n]` 占位，点击可临时显示，再点收起

### v1.7.9

- Excel 摸鱼外观适配全页搜索：结果行表格化、行号、悬停/选中、公式栏显示关键词与选中项
- 搜索页筛选条/输入框/按钮压成表格控件风格；分类标签与摘要弱化

### v1.7.5

- 修复 Excel 设置区下拉/输入框中文被裁切（加高加宽、恢复原生 select）

### v1.7.4

- 去掉无效「列表预加载」（Discourse 原生无限滚动，无加载更多按钮可劫持）
- 设置项去掉口水说明文案；Excel 子选项下拉高度修复（文字不再被裁切）
- 设置面板 / Excel 页面滚动条改为细条样式（不再强制隐藏）

### v1.7.3

- 去掉「外链直接跳转」：保留 linux.do 官方「打开外部链接」安全确认
- 去掉「折叠引用」：站内原生已有展开/收起
- 「触底自动加载」改为「列表预加载」：距底约 2 屏提前点「加载更多」，不抢话题页楼层滚动
- 「新标签打开帖子」默认关闭；开启后用捕获阶段点击绕过 Discourse SPA 劫持
...
- 高级设置：字体偏移、图片宽度、楼主颜色、拉黑模式、浮动按钮位置等
